    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div class="relative flex h-16 items-center justify-between">
            ...
            <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
                <button x-on:click="open = true" class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
            ...
            {{-- Con la directiva auth verificamos si el usuario esta autenticado --}}
            @auth   
                {{-- Si esta autenticado va a mostrar esto --}}
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    ...
                    <!-- Profile dropdown -->
                    {{-- Asignamnos el x-data y ponemos una variable (open) se le asigna 
                    false para que no se muestre --}}
                    <div class="relative ml-3" x-data="{open: false}">
                        ...
                        {{-- aqui asignamos al x-show que sea igual a la variable open que es igual a false
                        para que no se muestre el contenido y con x-on:click.away le decimos que cuando open sea 
                        true (se ejecuta el button de arriba que lo cambia a true), 
                        lo convierta en false para ocultarlo nuevamente --}}
                ...
                @else 
                {{-- Si no esta autenticado mostrará esto --}}
                <div>
